...
public class Eventual_Safe_States {
     public boolean DFS(int s, List<List<Integer>> graph, boolean[] visited, boolean[] dfsVisited,
               boolean[] checkCycle) {
          ...
          for (int it : graph.get(s)) {
               if (!visited[it]) { // If the adjacent node is not visited
                    if (DFS(it, graph, visited, dfsVisited, checkCycle))
                         return checkCycle[s] = true; // If a cycle is detected, mark the current node as part of a
                                                      // cycle
     ...
     public List<Integer> eventualSafeNodes(int[][] graph) {
          ...
          for (int i = 0; i < v; i++) {
               if (!checkCycle[i])
                    ans.add(i); // Add the nodes that are not part of any cycle (eventual safe nodes) to the
                                // result
          }
